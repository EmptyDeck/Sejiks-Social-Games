...
3. **너의 평점은: 플레이어 기기가 자기 번호를 모른다.** 서버가 없어 `playerTokens`
   가 비어 있고, `validateToken()` 폴백이 무조건 `1`을 넣는다. 모두가 "플레이어 1"로
   보인다. 비밀 숫자 자체는 코드에서 계산되므로 정확하다.
4. **너의 평점은: 남의 코드를 넣으면 그 사람 숫자가 보인다.** 코드→숫자가 순수 함수라
   서버 없이는 막을 수 없다.
